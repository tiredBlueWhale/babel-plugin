    ...
    if (node.operator === "+") {
        if (hasValueOfString(nodeRight)) {
            ...
            if (hasValueOfString(nodeLeft)) {
                returnArr.push(t.StringLiteral(stringValue(nodeLeft) + result));
            } else if (t.isBinaryExpression(nodeLeft)) {
                binaryExpression(nodeLeft, returnArr, result);
            } else {
                if (DEBUG_BINARY_EXPRESSION) cssToStyleError.push({ node: node, message: 'BINARY EXPRESSION::NODE LEFT' });
            }
        } else {
            if (DEBUG_BINARY_EXPRESSION) cssToStyleError.push({ node: node, message: 'BINARY EXPRESSION::NODE RIGHT' });
        }
    } else {
        if (DEBUG_BINARY_EXPRESSION) cssToStyleError.push({ node: node, message: 'BINARY EXPRESSION::OPERATOR ' + node.operator });
    }
}
...
const transformToValidExpression = node => {
    ...
    if (hasValueOfString(node)) {
        ...
        const binaryExpressionArray = []
        binaryExpression(node, binaryExpressionArray);
        binaryExpressionArray.forEach(expression => returnArr.push(...transformToValidExpression(expression)));
    }
    return returnArr;
}
...
const conditionalExpressionConsequentAlternate = (node, nodeDescription) => {
    ...
    if (resultingNode.length > 0) {
        ...
        if (DEBUG_CONDITINAL_EXPRESSION) cssToStyleError.push({ node: node, message: 'CONDITIONEL EXPRESSION ' + nodeDescription })
        return node;
    }
}
...
const conditionalExpression = (conditionalExpression) => {
    if (!(t.isMemberExpression(conditionalExpression.test)
        ...
        if (DEBUG_CONDITINAL_EXPRESSION) cssToStyleError.push({ node: conditionalExpression.test, message: 'CONDITIONEL EXPRESSION TEST' })
    }
...
const callExpression = (callExpression, styleArr) => {
    if (t.isMemberExpression(callExpression.callee)) {
        if (callExpression.callee.property.name === "join" && callExpression.arguments.value === " ") {
            ...
            if (DEBUG_CALL_EXPRESSION) cssToStyleError.push({ node: node, message: 'CALL EXPRESSION::PROPERTY ' + callExpression.callee.property.name })
            transformExpression(callExpression.callee.object, styleArr);
            // cssToStyleError.push({ node: callExpression.callee.object, message: 'CALLEXPRESSION ' + node.operator });
            // cssToStyleError.push({ node: callExpression.callee.property, message: 'RIGHT BINARY EXPRESSION ' + node.operator });
        }
    } else {
        if (DEBUG_CALL_EXPRESSION) cssToStyleError.push({ node: callExpression.callee, message: 'CALL EXPRESSION CALLEE UNKOWN' });
    }
}
...
const transformExpression = (node, styleArr) => {
    ...
    if (styles && styles.length > 0) {
        ...
        if (!(t.isTemplateElement(node) || t.isStringLiteral(node)) || DEBUG_ALL_EXPRESSIONS) {
            cssToStyleError.push({ node: node, message: 'transformExpression function' })
        }
    }
}
...
const processJSXElements = {
    ...
    MemberExpression(path) {
        ...
        const isExpression = (node, identifiers) => {
            ...
            if (node.property && node.property.name === identifer) {
        ...
        if (isExpression(path.node, ["nav", "userAgent", "toLowerCase"])) {
            // path.parent.node = t.booleanLiteral(false);
            // path.parent.replaceWithSourceString(`false`);
            path.parentPath.replaceWithSourceString(`false`);
            // console.log(path.parentPath.parent.type);
        }
        if (isExpression(path.node, ["window", "document", "createElement"])) {
            path.replaceWithSourceString(`React.createElement`);
